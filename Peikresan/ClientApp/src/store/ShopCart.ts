...
export interface IChangeShopCartItem { type: ShopCartActions, payload: {productId:number,count?:number} }
...
export const actionCreators = {
    addProduct: (productId:number ,count=1) => ({ type: ShopCartActions.ADD_PRODUCT,payload:{productId,count} } as IChangeShopCartItem),
    removeProduct: (productId:number ,count=1) => ({ type: ShopCartActions.REMOVE_PRODUCT,payload:{productId,count} } as IChangeShopCartItem),
    deleteProduct: (productId:number) => ({ type: ShopCartActions.DELETE_PRODUCT,payload:{productId} } as IChangeShopCartItem),
};

// ----------------
// REDUCER - For a given state and action, returns the new state. To support time travel, this must not mutate the old state.

export const reducer: Reducer<IShopCartState> = (state: IShopCartState | undefined, incomingAction: Action): IShopCartState => {
    if (state === undefined) {
        return { shopCart: [] };
    }
    ...
    switch (action.type) {
        ...
        case ShopCartActions.REMOVE_PRODUCT:
            ...
            if(removedShopCart[action.payload.productId]){
                removedShopCart[action.payload.productId] -= action.payload.count? action.payload.count: 0;
