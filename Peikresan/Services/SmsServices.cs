...
namespace Peikresan.Services
{
    public static class SmsServices
    {
        ...
        {
            var message = $"پیک‌رسان\nفروشنده گرامی سلام\nدر حساب کاربری شما یک سفارش به شماره {subOrderId} برای تحویل {time} به ثبت رسیده است. جهت آماده‌سازی و ارسال آن به مشتری به لینک زیر مراجعه کنید: https://peikresan.com/suborder/{Helper.EncodeNumber(subOrderId)}";
            return SendSms(mobile, message);
        }

        public static bool Sms2DeliveryAfterBuy(string mobile, int orderId, string time)
        {
            var message = $"پیک‌رسان\nراننده گرامی سلام\nسفارشی با کد پیگیری {orderId} برای تحویل {time} برای شما ثبت شده است.برای اطلاع بیشتر به لینک زیر مراجعه کنید : https://peikresan.com/suborder/{Helper.EncodeNumber(orderId)}";
            return SendSms(mobile, message);
        }

        public static bool Sms2CostumerAfterDeliveryGetProducts(string mobile, string name, int orderId, int deliveryCode)
        {
